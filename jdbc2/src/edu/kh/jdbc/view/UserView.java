...
/**
 * 3.User 검색어 조회(SELECT)
 * */
private void selectName() throws Exception{
	System.out.println("\n=== 3. User 중 이름에 검색어가 포함된 회원 조회 (SELECT) ===\n");
	
	//입력된 검색어와 일치하는 USER 정보 조회
	System.out.print("검색어 입력 : ");
	String keyword = sc.nextLine();
			
	//서비스(SELECT) 호출 후 결과 반환 받기
	List<User> searchList = service.selectName(keyword); //1명이 아닐수도 있어서 List로 받아야함
	...
	for(User user : searchList) {
		System.out.println(user); //user 객체 출력(toString())
	}
}
